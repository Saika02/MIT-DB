...
public class TupleDesc implements Serializable {
    ...
    public int fieldNameToIndex(String name) throws NoSuchElementException {
        ...
        for (int i = 0; i < numFields(); i++) {
            String f = tdItems.get(i).fieldName;
            if(f == null) throw new NoSuchElementException();
            if(f.equals(name)) return i;
        }
    ...
    public static TupleDesc merge(TupleDesc td1, TupleDesc td2) {
        ...
        Type[] newTypeAr = new Type[len];
        String[] newFieldAr = new String[len];
        for (int i = 0; i < len; i++) {
            if (i < td1.numFields()) {
                newTypeAr[i] = td1.getFieldType(i);
                newFieldAr[i] = td1.getFieldName(i);
            } else {
                newTypeAr[i] = td2.getFieldType(i - td1.numFields());
                newFieldAr[i] = td2.getFieldName(i - td1.numFields());
            }
        }
    ...
    public boolean equals(Object o) {
        ...
        if (o instanceof TupleDesc) {
            ...
            for (int i = 0; i < numFields(); i++) {
                if (!td.getFieldType(i).equals(getFieldType(i))) return false;
            }
            return true;
        }
        // some code goes here
